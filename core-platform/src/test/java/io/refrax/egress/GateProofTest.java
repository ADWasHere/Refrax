 ...
 * ({@code SensorReadResource.project}) copies only declared exposable fields into the
 * typed {@code ExposableReading}, so an internal field cannot appear in the output —
 * the value handed to the response simply does not contain one.
 *
...
class GateProofTest {
    ...
    void internalFieldsNeverLeakIntoProjection() {
        ...
        // Only the exposable part out.
        given()
                .when()
                .get("/v1/sensors/{sensorId}/latest", sensorId)
                .then()
                .statusCode(200)
                // Exposable fields are present and correct...
                .body("sensorId", equalTo(sensorId))
